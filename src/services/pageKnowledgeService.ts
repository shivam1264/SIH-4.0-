// ── Universal Page Knowledge & Intelligent Q&A Service ───────────────
// SIGHT-EXAM AI Accessibility Platform
// Answers ANY spoken question (in English, Hindi, or Hinglish) about what is written
// on any of the 9 sidebar pages, backed by instant local knowledge + Groq AI fallback.
...
export function resolvePageNameFromRoute(route: string, currentActivePage?: string): string {
  if (currentActivePage && SIDEBAR_PAGES_KNOWLEDGE[currentActivePage]) {
    return currentActivePage;
  }
  for (const [r, p] of Object.entries(ROUTE_TO_PAGE_MAP)) {
    if (route.startsWith(r)) return p;
  }
  return currentActivePage || 'Dashboard';
}
...
export function isQuestionOrQuery(text: string): boolean {
  if (!text || typeof text !== 'string') return false;
  const t = text.toLowerCase().trim();
  if (t.length < 2) return false;

  // Explicit Question words (Hindi, Hinglish, English)
  if (
    /\b(kya|kaise|kitna|kitne|kaun|kiska|kiske|kahan|kyu|kyun|batao|samjhao|sunao|dikhao|kon|konsa|konse|what|how|who|which|where|when|why|tell\s+me|explain|is\s+page|iss\s+page|about\s+this|details?|overview|summary)\b/i.test(t) ||
    /[\u0900-\u097F]/.test(t) ||
    t.endsWith('?') ||
    /^(tell|explain|describe|show|read|summarize)\b/i.test(t)
  ) {
    return true;
  }
...
/**
 * Ask question with live Groq AI, falling back to local deterministic knowledge engine.
 */
export async function askPageQuestion(
  ...
  const BACKEND_ASK_URL = 'http://localhost:5000/api/ai/ask-page';

  try {
    const res = await fetch(BACKEND_ASK_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ page, question, contextText }),
    ...
    console.warn('[PageKnowledge] Live AI backend unreachable, using local knowledge engine:', err);
  }

  return resolveLocalPageQuestion(page, question);
}
